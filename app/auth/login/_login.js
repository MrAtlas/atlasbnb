...
export default function LoginForm() {
  ...
  React.useEffect(() => {
    if (user && user.email) redirect('/homepage');
  }, [user]);
  ...
  return (
    ...
    >
      <LogoFull fill={'#00B96B'} style={{ width: '100%', height: 80 }} />
      <Row gutter={16}>
        <Col lg={12} md={24}>
          <Form
            labelCol={{
          ...
          >
            <Form.Item style={{ textAlign: 'left', paddingBottom: 15 }}>
              <h3 style={{ fontSize: 30, color: '#00804a' }}>Sign in</h3>
              <Typography.Text type="secondary">
                Don&apos;t have an account?{' '}
                <Typography.Link><Link href='/auth/register'>Sign up</Link></Typography.Link>
              </Typography.Text>
            </Form.Item>
            <Form.Item
              label="Email"
        ...
        <Col lg={12} md={24}>
          ...
          >
            ...
            <Button href='/auth/register' type="primary" shape="round" style={{ marginTop: 32 }}>
              Sign up
            </Button>
          </Flex>
        </Col>
